...
/** 文件历史条目：逐字段对应 contracts 的 FileHistoryEntry（core 层持 Core 前缀镜像，api 层映射） */
...
/** %H%x00%h%x00%s%x00%an%x00%aI —— 每条记录 5 字段、字段间 \x00，记录间换行 */
const HISTORY_FORMAT = '%H%x00%h%x00%s%x00%an%x00%aI';

/**
 * 解析 --format 的 NUL 分隔输出为条目数组。
 * 算法：记录以 \n 为界（git 每条记录以换行结尾，记录间无 NUL——NUL 只在字段间，
 * 每条记录 5 字段 4 个 NUL，按 NUL 直接分组会在记录边界错位）；
...
export function parseHistoryRecords(stdout: string): CoreFileHistoryEntry[] {
  ...
  for (const record of stdout.split('\n')) {
    ...
    const [hash, shortHash, subject, author, dateIso] = record.split('\0');
    if (hash === undefined || shortHash === undefined || subject === undefined || author === undefined || dateIso === undefined) continue;
    entries.push({ hash, shortHash, subject, author, dateIso: dateIso.replace(/\r$/, '') });
  }
  return entries;
}

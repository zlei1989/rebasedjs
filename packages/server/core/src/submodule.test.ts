...
function instantiateRig(tpl: Rig): Rig {
  ...
  const bareUrl = bare.replace(/\\/g, '/');
  // super/.gitmodules 的 url 与 super/.git/modules/<path>/config 的 origin url 均指向模板裸仓库，替换为新副本
...
describe('submodule 原语', () => {
  ...
  it('checked-out 往返：改检出 → different-commit → update 复原', { timeout: 180_000 }, async () => {
    const { super: superRepo, content, aSha, bSha, gitlinkSha } = instantiateRig(defaultRig!);

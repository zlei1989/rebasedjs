/** remote 原语测试：远程 CRUD、fetch/pull/push 各态、pushUpToCommit、本地独有提交、传输超时兜底、浅克隆检测。
 *  性能：裸仓库 + clone 装置（rig）在 beforeAll 建一次模板，用例复制 repo 与 bare 两目录并
 *  以文本替换修正 origin URL（0 spawn；本机单次 git spawn ~330ms，历史每 rig ~8 spawn）。 */
...
function instantiate(template: string): string {
  return track(instantiateFixture(template));
}
...
/** 复制 rig 模板：repo 与 bare 各复制一份，并文本替换 origin URL 指向新的 bare 副本（0 spawn）。
 *  gitconfig 值内反斜杠转义为双反斜杠存储，替换时须同样转义。 */
function instantiateRig(): { repo: string; bare: string; defaultBranch: string } {
...
describe('fetchRemote', () => {
  ...
  it(
    ...
    { timeout: RIG_TIMEOUT },
    async () => {
      const { repo, bare } = instantiateRig();
      ...
      expect(target).not.toBe(git(bare, ['rev-parse', 'HEAD', '--symbolic-full-name']));
...
describe('pushBranch', () => {
  ...
  it(
    ...
    { timeout: RIG_TIMEOUT },
    async () => {
      const { repo, defaultBranch } = instantiateRig();

...
export async function getLogPage(repoPath: string, query: LogQuery, opts: { signal?: AbortSignal } = {}): Promise<LogPage> {
  ...
  for await (const c of streamLog(repoPath, { skip: query.skip, maxCount: query.limit, author: query.author, path: query.path, range: query.range, all: query.all === true, signal: opts.signal })) {

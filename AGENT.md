...
- **写 UI 前先用 context7 查组件用法** — 常规界面与布局用 `antd`、数据可视化用 `@ant-design/plots`；适配`light`与`dark`主题，弹性布局自适应屏幕尺寸；样式一律走 `antd`（主题 token / 紧凑密度 / 语义 `styles`）——**不手写字号**（交紧凑密度）、**不手调行内边距**（交组件默认），避免裸写 `div` 等原始标签
...
- **建仓用 `createTmpRepo()`**（模板复制，0 spawn）；禁止测试内 `git init` 造夹具
- **同构夹具 `beforeAll` 建一次模板、用例 `instantiateFixture()` 复制**；禁止每用例重建

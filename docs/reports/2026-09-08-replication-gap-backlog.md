...
### 2.5 工程排期项（11 项）—— ✅ 全部完成（随批消化，2026-09-21 收官）

gitlab checkout Bearer 注入 hardening→✅ 已落地（`buildAuthConfig` 域分支：gitlab.com → `Authorization: Basic base64("oauth2:<token>")`——GitLab git-over-HTTP 不识别 Bearer，官方 `https://oauth2:TOKEN@` 的等价头形式；其余 host 维持 Bearer；api +1 单测）；web-next 空/非法 JSON body 500 与 koa 400 全局评估→✅（两端 handleApiError SyntaxError → 400，见 §2.6）；createOpen 跨仓库保持打开→✅ 已落地（`<LogPage key={repoId}>` 重挂载复位内嵌 Modal（创建分支/标签/Reword）+ 两端容器 repoId 切换复位 effect——选中提交/push-up-to/打开对话框/认证重试/更新结果面板/变更集/过滤条件全清）；worktree 回滚失败包 gitFailure→✅ 已落地（post-add 归一校验的回滚包裹 gitFailure「回滚无效工作树失败（条目可能残留）」）；resolveSubmodulePath 白名单→✅ 已落地（path 必须解析在仓库根内——`.gitmodules` 声明 `../x` 越界 → 拒绝「子模块路径越界」；core +1 单测）；core vitest `fileParallelism`→✅（外部提交 5ac5fb0 已调并行度与夹具模板化，见 core vitest.config）；unborn HEAD 建补丁/搁置→✅ 已落地（core `isUnbornHead`/`collectWorkingDiff`——空仓库两段拼接（`git diff --cached` vs 空树 + `git diff` vs 索引）；patch.createPatch 工作区模式 unborn 分支 + 单测、shelf.saveShelf 同源复用 + 单测）；execLogByCwd 仓库级淘汰→✅ 已落地（LRU：目录上限 64，写/读访问提升，超限逐出最久未访问；core +1 单测（70 目录验证淘汰与重访提升））；面板 key `kind+id`→✅ 已落地（GitHub/GitLab 时间线行 key 改为 `${entry.kind}-${entry.id}`——跨类型事件 id 域独立，防 React key 碰撞）；addIgnore path 字符集收紧→✅ 已落地（换行/回车/控制字符 → INVALID_QUERY「忽略路径不合法」——防经 path 注入多行 .gitignore 规则；api +1 单测）；存档名 `.`/`..` 边界统一→✅ 已落地（`assertValidEntryName` 补丁/搁置共用：禁空名/`.`/`..`/`/`与`\`——Windows 反斜杠即分隔符注入，逃逸名由「按不存在处理」收紧为 INVALID_QUERY；core +2、api +3 单测）。

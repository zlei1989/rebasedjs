...
/**
 * 应用（全局）设置页容器（web-next）：useSettings + 账户三 hooks + git 可执行文件检测注入 ui AppSettingsPage。
 * 作用域：本页全部项与 repoId 无关（写入应用配置 ~/.rebasedjs/config.json），对所有仓库生效；
 * 仓库级项在 /repos/:repoId/settings（仓库设置页），两页互跳。
 * 互跳到仓库设置需要「最近仓库」：取列表第一条（服务端按打开时间降序，与首页 Configure 既有口径同源）。
...
import { AppSettingsPage } from '@rebased/ui';
...
export default function Page(): React.ReactNode {
  ...
  // 最近仓库（打开时间降序的第一条）：互跳仓库设置的落点；无仓库时链接禁用（无从指定配置哪个仓库）
  ...
  return (
    <AppSettingsPage
      ...
      onOpenOtherSettings={
        latestRepoId === undefined ? undefined : (): void => router.push(`/repos/${latestRepoId}/settings`)
      }
    />
  );
}

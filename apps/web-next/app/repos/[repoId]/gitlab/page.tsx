 ...
 * 不发请求）+ 五个 mutation 注入 ui GitLabPanel（与 web-koa 容器同构）。
 * 仓库顶栏导航（RepoTopNav）取代「返回日志」链接；「刷新」按钮重取 mrs/detail/timeline/files 四键（照 P3-D console 页做法，
    ...
    <PageShell>
      ...
      <Flex gap={8} style={{ alignSelf: 'flex-start' }}>
        <Tooltip title="打开应用设置：GitLab 令牌配在「账户」卡片（账户是应用级资源，对所有仓库生效）">
          <Button type="link" data-testid="gitlab-open-settings" onClick={() => router.push('/settings')}>
            设置
          </Button>
        </Tooltip>
      </Flex>

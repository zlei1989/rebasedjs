...
import { abortOperation, addGithubPrComment, addGithubPrReviewComment, addGitlabMrComment, addGitlabMrDiscussion, addIgnore, amendSpecificCommit, applyAutosquash, applyBranchAction, applyChangelistAction, applyCheckout, applyHunkStaging, applyPatchService, applyRemoteAction, applyReset, applyShelfAction, applyStaging, applyStashAction, applyTagAction, checkoutGithubPr, checkoutGitlabMr, checkoutRebase, checkoutUpdate, cherryPick, cloneRepo, commitAndPush, commitEdit, continueMergeOperation, continueOperation, createCommit, createGitlabMr, createPatch, createWorktree, deleteAccount, deletePatch, fetchRepo, forcePushedUpdate, getAmendTargets, getAppHomeDir, getBranchWorkingDiff, getBranches, getBrowseContent, getBrowseTree, getChangelists, getCommitFiles, getConflictContents, getConflicts, getConsole, getCrlfWarning, getFileBlame, getFileDiff, getFileHistory, getFileThreeVersions, getGitExecutableInfo, getGpgSettings, getIgnore, getIgnoreTemplates, getLogPage, getOperation, getPatches, getRebaseTodo, getRemotes, getShelves, getRepoConfig, getRepoStatus, getSettings, getFileVersions, getGithubPrDetail, getGithubPrFiles, getGithubPrReviewComments, getGithubPrs, getGithubPrTimeline, getGithubStatus, getGitlabMrDetail, getGitlabMrDiscussions, getGitlabMrFiles, getGitlabMrs, getGitlabMrTimeline, getGitlabStatus, getStashDiff, getStashes, getSubmodules, getTags, getWorktrees, importPatchIntoShelf, initRepo, listAccounts, listRecentRepos, mergeBranchIntoCurrent, mergeGithubPr, mergeGitlabMr, openRepo, pruneWorktrees, pullRepo, pushRepo, putIgnore, rebaseBranch, removeRepo, removeWorktree, resolveConflict, revert, runInteractiveRebaseService, searchCommitsService, setGpgSettings, setRepoConfig, skipOperation, streamDiffEvents, streamLogEvents, submitGithubPrReview, submitGitlabMrReview, undoCommit, unstashAs, updateProject, updateSettings, updateSubmodules, upsertAccount, watchRepoStatus } from '@rebased/api';
import { accountBodySchema, accountDeleteBodySchema, amendSpecificBodySchema, autosquashBodySchema, blameQuerySchema, branchActionSchema, branchWorkingDiffQuerySchema, browseContentQuerySchema, browseQuerySchema, changelistActionSchema, checkoutActionSchema, checkoutRebaseBodySchema, checkoutUpdateBodySchema, cloneRepoBodySchema, commitAndPushBodySchema, commitBodySchema, commitEditBodySchema, configPutBodySchema, conflictContentsQuerySchema, consoleQuerySchema, diffQuerySchema, fetchBodySchema, githubCommentBodySchema, githubMergeBodySchema, githubPrNumberSchema, githubPrQuerySchema, githubReviewBodySchema, githubReviewCommentBodySchema, gitlabCommentBodySchema, gitlabDiscussionBodySchema, gitlabMergeBodySchema, gitlabMrCreateBodySchema, gitlabMrIidSchema, gitlabMrQuerySchema, gitlabReviewBodySchema, gpgConfigBodySchema, historyQuerySchema, hunkStagingBodySchema, ignoreAddBodySchema, ignorePutBodySchema, initRepoBodySchema, interactiveRebaseBodySchema, logQuerySchema, mergeBodySchema, openRepoBodySchema, patchApplyBodySchema, patchCreateBodySchema, patchDeleteBodySchema, patchImportShelfSchema, pickBodySchema, pullBodySchema, pushBodySchema, rebaseBodySchema, rebaseTodoQuerySchema, remoteActionSchema, resetBodySchema, resolveConflictBodySchema, searchQuerySchema, serializeSseEvent, settingsPatchSchema, shelfActionSchema, stagingBodySchema, stashActionSchema, stashIndexSchema, stashUnstashAsBodySchema, submoduleUpdateBodySchema, tagActionSchema, threeWayQuerySchema, updateBodySchema, worktreeCreateBodySchema, worktreeRemoveBodySchema, type SseEvent } from '@rebased/contracts';
import { z } from 'zod';
...
/** GET /api/repos/:repoId/settings/gpg-config —— GPG 提交签名配置（GitGpgConfigDialog 语义）：启用态 + 选定密钥 + 可用密钥列表 → 200 GpgConfigView */
router.get('/api/repos/:repoId/settings/gpg-config', async (ctx) => {
  try {
    ctx.body = await getGpgSettings(resolveRepo(z.string().min(1).parse(ctx.params.repoId)));
  } catch (error) {
    handleApiError(error, ctx);
  }
});

/** PUT /api/repos/:repoId/settings/gpg-config —— zod 校验（enabled=true 须带 key）→ setGpgSettings（写 commit.gpgsign/user.signingkey）→ 200 刷新视图 */
router.put('/api/repos/:repoId/settings/gpg-config', async (ctx) => {
  try {
    const body = gpgConfigBodySchema.parse(ctx.request.body);
    ctx.body = await setGpgSettings(resolveRepo(z.string().min(1).parse(ctx.params.repoId)), body);
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
/** GET /api/repos/:repoId/diff/branch-working —— zod 校验查询（branch）→ getBranchWorkingDiff（分支 vs 工作树：
 *  git diff <ref> --name-status 文件清单，GitShowDiffWithRefAction 语义）→ 200 BranchWorkingDiff；无效分支 → 400 INVALID_REF */
router.get('/api/repos/:repoId/diff/branch-working', async (ctx) => {
  try {
    const query = branchWorkingDiffQuerySchema.parse(ctx.query);
    ctx.body = await getBranchWorkingDiff(resolveRepo(z.string().min(1).parse(ctx.params.repoId)), query.branch);
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
/** POST /api/repos/:repoId/commit —— zod 校验请求体 → createCommit（缺 user.name/email → 400 INVALID_QUERY；crlfFix 先写 autocrlf 建议值）→ 200 {hash} */
...
router.get('/api/repos/:repoId/commit/crlf-warning', async (ctx) => {
  try {
    ctx.body = await getCrlfWarning(resolveRepo(z.string().min(1).parse(ctx.params.repoId)));
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
router.get('/api/repos/:repoId/commit/amend-targets', async (ctx) => {
  try {
    ctx.body = await getAmendTargets(resolveRepo(z.string().min(1).parse(ctx.params.repoId)));
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
router.post('/api/repos/:repoId/commit/amend-specific', async (ctx) => {
  try {
    const body = amendSpecificBodySchema.parse(ctx.request.body);
    ctx.body = await amendSpecificCommit(resolveRepo(z.string().min(1).parse(ctx.params.repoId)), body);
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
router.post('/api/repos/:repoId/autosquash', async (ctx) => {
  try {
    const body = autosquashBodySchema.parse(ctx.request.body);
    ctx.body = await applyAutosquash(resolveRepo(z.string().min(1).parse(ctx.params.repoId)), body);
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
router.post('/api/repos/:repoId/commit-edit', async (ctx) => {
  try {
    const body = commitEditBodySchema.parse(ctx.request.body);
    ctx.body = await commitEdit(resolveRepo(z.string().min(1).parse(ctx.params.repoId)), body);
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
router.post('/api/repos/:repoId/checkout-rebase', async (ctx) => {
  try {
    const body = checkoutRebaseBodySchema.parse(ctx.request.body);
    ctx.body = await checkoutRebase(resolveRepo(z.string().min(1).parse(ctx.params.repoId)), body);
  } catch (error) {
    handleApiError(error, ctx);
  }
});
 ...
 *  fetch 跟踪分支 + merge/--rebase）→ 200 RebaseOutcome；无上游/当前分支 → 400 INVALID_QUERY；分支不存在 → 400 INVALID_REF */
router.post('/api/repos/:repoId/checkout-update', async (ctx) => {
  try {
    const body = checkoutUpdateBodySchema.parse(ctx.request.body);
    ctx.body = await checkoutUpdate(resolveRepo(z.string().min(1).parse(ctx.params.repoId)), body);
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
router.post('/api/repos/:repoId/update/force-pushed', async (ctx) => {
  try {
    ctx.body = await forcePushedUpdate(resolveRepo(z.string().min(1).parse(ctx.params.repoId)));
  } catch (error) {
    handleApiError(error, ctx);
  }
});
...
router.get('/api/settings/git-executable', async (ctx) => {
  try {
    ctx.body = await getGitExecutableInfo();
  } catch (error) {
    handleApiError(error, ctx);
  }
});

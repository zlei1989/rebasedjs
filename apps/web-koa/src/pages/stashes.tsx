/**
 * 贮藏页容器：useStashes + useStashAction + useStashUnstashAs + useStashDiff + useBranches 注入 ui StashPanel（与 web-next 容器同构；
 * repoId 取 useParams、返回导航用 useNavigate，而非 Next params/router）。操作失败经 message.error 呈现
 * （成功响应由各 hook 显式回写缓存）；Unstash As 会检出目标分支 → 成功后重验证 status 键；
